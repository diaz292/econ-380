...
\begin{questions}
	...
	\begin{parts}
		...
		\ddp{Points: (1) $w^*$, (1) $E^*$. Minus $1/2$ if units are wrong or missing.}
		\end{solution}
		...
		\end{solution}
		\ddp{Points: (2) for correctly identifying $PS$ and $WS$ on the graph, (1) each for numerical values of $PS$ and $WS$. Minus $1/2$ for incorrect units on either, but only need to take off $1/2$ even if both have incorrect units.}
		...
		\begin{solution}[1in]
		 The minimum wage is above the equilibrium market wage, so it would be binding. With the \$20 minimum wage, $E_D = 300 - 10(20) = 100$, so only 100 million workers will be employed ($\bar{E} = 100$ million). Plug in 100 into $E_S$ to find reservation wage of the 100 millionth worker: $100 = 30w - 300 \Rightarrow w = 13.3$ 
		 ...
		 \end{figure}
		\end{solution}
	\ddp{The answers here may differ somewhat from mine depending on their rounding, but it shouldn't be too far off. Make sure they give the \underline{change} in PS, WS, and TS. If they just gave the new values (and got them right), they can get half credit. Don't worry about units here as long as they are consistent.}
	\end{parts}
\end{questions}
