...
\begin{center}
	\fbox{\fbox{\parbox{5.5in}{\centering
				This homework is due on \textbf{December 6} by \textbf{2:25PM}. You must turn in your work on a printed copy of this document in order for it to be graded. Your assignment must be stapled and in the correct order. Non-stapled assignments will automatically receive a 10 point deduction.}}}
\end{center}

\begin{questions}
	
\question Consider an economy with a population of 100,000 households. 60 percent of households report an annual income of \$25,000 (``low earners''), 30 percent of households report an annual income of \$70,000 (``mid earners''), and 10 percent report an annual income of \$110,000 (``high earners'').

\begin{parts}
\part[2] What is the total income earned by households in this economy?
\begin{solution}[1in]
Total Income = 	\[\underbrace{100,000\times.60\times\$25,000}_{\text{Low income group} = \$1.5B} + \underbrace{100,000\times.30\times\$70,000}_{\text{middle income group} = \$2.1B} + \underbrace{100,000\times.10\times\$110,000}_{\text{high income group} = \$1.1B}\]

\[= 100,000\times [(.60\times \$25,000) + (.30\times \$70,000) + (.10\times \$110,000)]\]

\[= \$4.7B\]
\end{solution}
\part[2] What is the share of total income accruing to the low earner households?
\begin{solution}[.75in]
1.5B/4.7B = 32\%
\end{solution}
\part[2] What is the share of total income accruing to the mid earner households?
\begin{solution}[.75in]
2.1B/4.7B = 45\%	
\end{solution}
\part[2] What is the share of total income accruing to the high earner households?
\begin{solution}[.75in]
1.1B/4.7B = 23\%	
\end{solution}


\part[4] Sketch the Lorenz curve for this economy as well as the perfect equality Lorenz curve.
\begin{solution}[3in]
	\begin{figure}[H]
	\centering
	\includegraphics[scale=.5]{lorenz}
\end{figure}
\end{solution}

\ddp{(1) labeled axes, (1) perfect equality Lorenz curve (diagonal from (0,0) to (100\%,100\%), (1) pt. at (60\%,32\%), (1) pt. at (90\%, 77\%)}

\part[5] What is the Gini coefficient associated with this economy? Make sure to show your work.
\begin{solution}[3in]
\[A = .5(.60)(.32) = .096\]	
\[B = .5(.30)(.45) = .0675\]
\[C = .5(.10)(.23) = .0115\]
...
Gini coefficient = $2(.5 - (A+B+C+D+E)) = 2(.5-.348) = .304$.
\end{solution}

\ddp{(2) correct Gini coefficient, (1) work, (1) correct areas calculated (even if graph is wrong)}

\uplevel{Suppose the number of households in the population doubled to 200,000.}

	\part[2] What is the total income earned by households in this economy?
	\begin{solution}[1in] Total income =
		\[200,000\times [(.60\times \$25,000) + (.30\times \$70,000) + (.10\times \$110,000)]\]
		\[= \$9.4B\]
	

	\end{solution}
	\part[3] What is the share of total income accruing to the each household income group?
	\begin{solution}[.75in]
		32\%, 45\%, and 23\% for the low, middle, and high income groups, respectively. 
		
		Notice that the total population size only changes the total income earned in the economy, but not the relative share going to each group (i.e., total population size doesn't matter when calculating the Gini coefficient).
		
		\ddp{No need to explain reasoning.}
	\end{solution}
		
	\part[2] What is the Gini coefficient for this economy now?
	\begin{solution}[.75in]
		Relative share of income stayed the same, so Gini coefficient is still .304.
	\end{solution}
